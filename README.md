...
アプリケーションUI向けの汎用SVGアイコン集です。1,034種類の各アイコンについてカラー版とモノクロ版を1点ずつ、合計2,068 SVGを収録しています。
...
- コード編集カテゴリに中間インデント揃え、インデント正規化、コード整形、選択範囲整形、複数行展開、1行統合の6種を追加
...
Lucide Icons 1.8.0の形状を基礎に、分類、配色、メタデータ、命名、ペア構造を本セット向けに構成しています。命名規則3種、UI部品25種、ファイル操作3種、コード整形6種は独自形状です。第三者由来部分の利用条件は `THIRD_PARTY_NOTICES.md` と `licenses/LUCIDE-LICENSE.txt` を参照してください。

## 収録構成

```text
universal-ui-icons/
├─ color/                 カラー版（1,034 SVG）
├─ monochrome/            モノクロ版（1,034 SVG）
├─ catalog/
...
- `UUI-CDE-0026` `align-column` — 中間インデント揃え
- `UUI-CDE-0027` `normalize-indent` — インデント正規化
- `UUI-CDE-0028` `format-code` — コード整形
- `UUI-CDE-0029` `format-selection` — 選択範囲整形
- `UUI-CDE-0030` `split-lines` — 複数行へ展開
- `UUI-CDE-0031` `join-lines` — 1行へ統合
