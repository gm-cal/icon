...
アプリケーションUI向けの汎用SVGアイコン集です。1,025種類の各アイコンについてカラー版とモノクロ版を1点ずつ、合計2,050 SVGを収録しています。
...
Lucide Icons 1.8.0の形状を基礎に、分類、配色、メタデータ、命名、ペア構造を本セット向けに構成しています。命名規則3種とUI部品25種は独自形状です。第三者由来部分の利用条件は `THIRD_PARTY_NOTICES.md` と `licenses/LUCIDE-LICENSE.txt` を参照してください。

## 収録構成

```text
universal-ui-icons/
├─ color/                 カラー版（1,025 SVG）
├─ monochrome/            モノクロ版（1,025 SVG）
├─ catalog/
